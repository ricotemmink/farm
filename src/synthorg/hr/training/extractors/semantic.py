"""Semantic memory content extractor.

Queries the memory backend for semantic-category entries from
...
class SemanticMemoryExtractor:
    """Extract semantic memory entries from senior agents.

    Queries the memory backend for ``SEMANTIC`` category entries
    ...
    async def extract(
        ...
        *,
        ...
        """Extract semantic memories from source agents in parallel.
        ...
        Returns:
            Unranked semantic training items.
        """
    ...
    async def _retrieve_for_agent(
        ...
        """Retrieve semantic entries for a single agent with error logging."""
        try:
