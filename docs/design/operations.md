...
- **Model discovery**: `POST /api/v1/providers/{name}/discover-models`
  ...
  - Accepts an optional `preset_hint` query parameter (`?preset_hint={preset_name}`) that guides endpoint selection and enables SSRF bypass for trusted preset URLs (e.g. `host.docker.internal`). The bypass requires both a valid `preset_hint` and that `base_url` matches the preset's `candidate_urls`.
  - Auto-triggered on preset creation for no-auth providers with empty model lists.
  - SSRF-safe URL validation applied before outbound requests; bypassed for no-auth local providers or validated preset-hinted URLs where localhost/private IPs are expected by design. Bypasses are logged at WARNING level (`PROVIDER_DISCOVERY_SSRF_BYPASSED`).
...
| `GET /api/v1/setup/status`, `GET /api/v1/setup/templates`, `POST /api/v1/setup/company`, `POST /api/v1/setup/agent`, `GET /api/v1/setup/agents`, `PUT /api/v1/setup/agents/{index}/model` (`{index}` = zero-based position in the list returned by `GET /api/v1/setup/agents`; not a stable ID -- re-fetch to resolve; out-of-range returns 404), `POST /api/v1/setup/complete` | First-run setup wizard: status check (public, reports `has_company`/`has_agents`/`has_providers` for step resume), template listing, company creation (auto-creates template agents with model matching), agent listing + model reassignment, manual agent creation (blank path), completion gate (requires company + agent + provider) |
