...
- **Settings** (`/settings`): Configuration for 7 UI-visible namespaces (api, memory, budget, security, coordination, observability, backup). Namespace tab bar navigation with single-column layout, basic/advanced mode, GUI/Code edit toggle (split-pane diff view for JSON/YAML). Observability sinks sub-page (`/settings/observability/sinks`) for log sink management with card grid and test-before-save. Backup management CRUD nested under backup namespace. System-managed settings hidden from GUI. Environment-sourced settings read-only.
    - *DB-backed persistence*: 17 namespaces total. User-facing namespaces: api, company, providers, memory, budget, security, coordination, observability, backup (company and providers are managed on their own dedicated pages). Bridged / operator-only namespaces: engine, communication, a2a, integrations, meta, notifications, tools, settings (surface the ~40 previously-hardcoded timeouts, batch sizes, and resource limits as ADVANCED settings; all default to ADVANCED level and are hidden from the basic UI). Setting types: `STRING`, `INTEGER`, `FLOAT`, `BOOLEAN`, `ENUM`, `JSON`. 4-layer resolution: DB > env > YAML > code defaults. Fernet encryption for `sensitive` values. REST API (`GET`/`PUT`/`DELETE` + schema endpoints for dynamic UI generation), change notifications via message bus.
    ...
    - *Bridge config composed reads*: For subsystems where several operator-tunable knobs live together (bus bridge, webhook bridge, OAuth flows, notification adapters, audit-chain signing, Docker sandbox sidecar, NATS history scans, dispatcher self-config, etc.), `ConfigResolver` exposes `get_<namespace>_bridge_config()` helpers (`get_api_bridge_config`, `get_communication_bridge_config`, `get_a2a_bridge_config`, `get_engine_bridge_config`, `get_memory_bridge_config`, `get_integrations_bridge_config`, `get_meta_bridge_config`, `get_notifications_bridge_config`, `get_tools_bridge_config`, `get_observability_bridge_config`, `get_settings_dispatcher_bridge_config`). Each returns a frozen Pydantic `BaseModel` (defined in `synthorg.settings.bridge_configs`, `ConfigDict(frozen=True, allow_inf_nan=False)`) whose field bounds mirror the underlying `SettingDefinition` bounds exactly, resolved in parallel via `asyncio.TaskGroup`. Defaults on each bridge model match the historical hardcoded value, so consumers constructed without a resolver (unit tests, degraded startup) keep the built-in behaviour.
    - *Hot-reload*: `SettingsChangeDispatcher` polls the `#settings` bus channel and routes change notifications to registered `SettingsSubscriber` implementations. Settings marked `restart_required=True` are filtered (logged as WARNING, not dispatched). Concrete subscribers: `ProviderSettingsSubscriber` (rebuilds `ModelRouter` on `routing_strategy` change via `AppState.swap_model_router`), `MemorySettingsSubscriber` (advisory logging for non-restart memory settings), `BackupSettingsSubscriber` (toggles `BackupScheduler` on `enabled` change, reschedules interval on `schedule_hours` change). Bridged settings fall into two tuning patterns: (1) per-iteration read (bus/webhook bridges cache the poll timeout + error budget at the top of each polling iteration, ticket cleanup resolves the interval per sleep, API controllers resolve query caps per request) so operator changes take effect without a restart; (2) bootstrap-only read (`restart_required=True`) -- the API startup hook resolves the setting once and threads the value into a mutable store via a setter. Two flavours of setter: **immediate application** -- the setter writes the resolved value straight onto a mutable instance attribute that later reads observe (e.g. `WsTicketStore.set_max_pending_per_user(value)` sets `self._max_pending`; `AuditChainSink.set_signing_timeout_seconds(value)` sets `self._signing_timeout_seconds` which `emit()` reads on the next record); **resolver injection** -- the setter stores the `ConfigResolver` handle on the instance and defers the actual lookup to a later async call site (e.g. `OAuthTokenManager.set_config_resolver(resolver)` stores the resolver; the timeout is resolved inside `OAuthTokenManager.start()` when the refresh loop spins up). The notification dispatcher takes the third approach: the startup hook resolves the `NotificationsBridgeConfig`, rebuilds the dispatcher with timeout-aware sink constructors, and swaps the new dispatcher into `AppState` via `AppState.set_notification_dispatcher(...)`, closing the pre-startup dispatcher's sinks afterwards. In all three cases the lookup is idempotent per `AppState` lifetime (`AppState.bridge_config_applied` one-shot guard).
...
The web image runs **Caddy** inside a pure-apko Wolfi image. Caddy serves the React SPA
at `/`, the built documentation at `/docs`, proxies REST requests at `/api/` and WebSocket
connections at `/api/v1/ws` to the backend, and emits a per-request CSP nonce via the
`templates` directive + `{http.request.uuid}` placeholder. The full security-header set
(CSP, HSTS, X-Frame-Options, Referrer-Policy, Permissions-Policy) is configured in
`web/Caddyfile`. Pre-compressed `.gz` siblings built by melange are served via Caddy's
`precompressed gzip` file_server option.
