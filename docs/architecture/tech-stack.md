...
| **Message Bus** | Internal (async queues), NATS JetStream (opt-in), Redis/RabbitMQ/Kafka planned | Pull-model `MessageBus` protocol with a pluggable backend factory. In-memory `asyncio` queues ship as the default for single-process deployments. NATS JetStream is the first distributed backend for multi-process and multi-host deployments ([Distributed Runtime design](../design/distributed-runtime.md)). Redis Streams, RabbitMQ, and Kafka are documented future backends. |
