...
3. **Header.** `web/security-headers.conf` emits the CSP with a matching nonce:
   `style-src-elem 'self' 'nonce-$csp_nonce'; style-src-attr 'unsafe-inline'`.
4. **Runtime read.** On page load, `web/src/lib/csp.ts` (`getCspNonce()`) reads the meta tag,
   rejects the un-substituted `__CSP_NONCE__` placeholder (so local dev where nginx isn't in
