...
src/synthorg/
  ...
  persistence/    # Pluggable PersistenceBackend, SQLite, settings + user + artifact + project + preset + workflow definition + workflow execution + workflow version + fine-tune repositories, artifact content storage (pluggable ArtifactStorageBackend, filesystem impl)
