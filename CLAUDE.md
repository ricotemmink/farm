...
src/synthorg/
  ...
  budget/         # Cost tracking, budget enforcement (pre-flight/in-flight checks, auto-downgrade), billing periods, cost tiers, quota/subscription tracking, quota degradation (degradation.py: DegradationResult, PreFlightResult, FALLBACK/QUEUE/ALERT strategy resolution via resolve_degradation, enforcer returns PreFlightResult from check_can_execute), CFO cost optimization (anomaly detection, efficiency analysis, downgrade recommendations, approval decisions), spending reports, budget errors (BudgetExhaustedError, DailyLimitExceededError, QuotaExhaustedError with provider_name/degradation_action context)
  ...
  observability/  # Structured logging (8-sink pipeline: console + 7 file sinks with logger-name routing), correlation tracking (request_id/task_id/agent_id via contextvars), sensitive field redaction, SYNTHORG_LOG_LEVEL env var override, critical sink enforcement (audit.log/access.log), third-party logger taming (_tame_third_party_loggers: clears LiteLLM/httpx/httpcore/aiosqlite/uvicorn/anyio/multipart handlers, sets WARNING + propagate=True, suppresses litellm.set_verbose/suppress_debug_info to eliminate duplicate Docker log output), log sinks, events/degradation.py (FALLBACK/QUEUE/ALERT event constants)
  providers/      # LLM provider abstraction (LiteLLM adapter), auth types (AuthType enum: api_key/oauth/custom_header/none), presets (ProviderPreset with candidate_urls for auto-detection, PROVIDER_PRESETS for Ollama/LM Studio/OpenRouter/vLLM, get_preset lookup), model auto-discovery (discovery.py: Ollama /api/tags + standard /models endpoint probing, SSRF-safe URL validation with trust_url bypass for preset/local-provider URLs, preset URL probing via probe_preset_urls/ProbeResult), runtime CRUD (management/ -- ProviderManagementService, asyncio.Lock-serialized create/update/delete/test/discover-models, hot-reload of ProviderRegistry + ModelRouter via AppState swap), provider family utilities (family.py: get_family, providers_excluding_family -- cross-family model selection for LLM security evaluation)
  ...
  security/       # SecOps agent, rule engine (soft-allow/hard-deny, fail-closed), audit log, output scanner, output scan response policies (redact/withhold/log-only/autonomy-tiered), risk classifier, risk tier classifier, action type registry, ToolInvoker security integration, progressive trust (4 strategies: disabled/weighted/per-category/milestone), autonomy levels (presets, resolver, change strategy), timeout policies (park/resume), timeout scheduler (ApprovalTimeoutScheduler -- background polling of pending approvals against timeout policies), LLM fallback evaluator (llm_evaluator.py: LlmSecurityEvaluator -- cross-family LLM evaluation for uncertain rule engine verdicts, EvaluationConfidence HIGH/LOW, tool-call structured output, configurable error policy via LlmFallbackConfig, configurable reason visibility (VerdictReasonVisibility: full/generic/category), configurable argument truncation strategy (ArgumentTruncationStrategy: whole_string/per_value/keys_and_values), XML-delimited prompt to resist injection)
