...
src/synthorg/
  ...
  providers/      # LLM provider abstraction (LiteLLM adapter), auth types (AuthType enum: api_key/oauth/custom_header/none), presets (ProviderPreset, PROVIDER_PRESETS for Ollama/LM Studio/OpenRouter/vLLM), model auto-discovery (discovery.py: Ollama /api/tags + standard /models endpoint probing, SSRF-safe URL validation), runtime CRUD (management/ -- ProviderManagementService, asyncio.Lock-serialized create/update/delete/test/discover-models, hot-reload of ProviderRegistry + ModelRouter via AppState swap)
