...
var versionCommentOnlyCases = []struct {
	name     string
	existing string
	fresh    string
	want     bool
	...
	{
		name:     "only first has newline",
		existing: "line1\nrest",
		fresh:    "no newline here",
		want:     false,
	},
}

func TestIsVersionCommentOnly(t *testing.T) {
	for _, tt := range versionCommentOnlyCases {
		t.Run(tt.name, func(t *testing.T) {
			got := isVersionCommentOnly([]byte(tt.existing), []byte(tt.fresh))
			if got != tt.want {
				t.Errorf("isVersionCommentOnly() = %v, want %v", got, tt.want)
			}
		})
	}
}

func FuzzIsVersionCommentOnly(f *testing.F) {
	f.Add("# v1\nrest", "# v2\nrest")
	f.Add("", "")
	...
	f.Fuzz(func(t *testing.T, existing, fresh string) {
		// Should not panic on any input.
		_ = isVersionCommentOnly([]byte(existing), []byte(fresh))
	})
}
